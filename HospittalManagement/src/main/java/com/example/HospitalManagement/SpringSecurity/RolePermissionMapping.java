...
import static com.example.HospitalManagement.Enums.PermissionsType.*;
import static com.example.HospitalManagement.Enums.RolesType.*;

public class RolePermissionMapping {

    // Role ko permission se map kr rahe hai 😌
    private static final Map<RolesType,Set<PermissionsType>> map = Map.of(     //---> Role or Uske permissions ka set
    ...
    public static Set<SimpleGrantedAuthority> getAuthoritieforRole(RolesType role) {  //---> Ek role lo Uske permissions nikalo Unko GrantedAuthority me convert karo
        return map.get(role).stream()  //--> Role ke permissions ka set le rahe ho.
              .map(permissionType -> new SimpleGrantedAuthority(permissionType.getPermissions()))  //--> Yaha conversion ho raha hai,PermissionsType enum → actual string authority
